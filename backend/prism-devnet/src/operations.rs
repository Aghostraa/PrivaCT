...
pub async fn add_data(
    ...
    let account = match prover.get_account(&user_id).await? {
        Found(account, _) => *account,  // Dereference the Box<Account>
        _ => {
            // If account doesn't exist, create it first
            create_account(user_id.clone(), prover.clone()).await?
        }
    };

    // Create the AddData operation
